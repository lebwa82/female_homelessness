...
1. Получить актуальный IPv4 test VM по ID `epdqi9moqfn8fqbe3g2n` через `uv run python -m scripts.resolve_prod_host --ip-only`, затем выполнить preflight: CPU/RAM/disk, Podman, firewall, свободные 80/443 и статус legacy service. Не выводить secret-files.
