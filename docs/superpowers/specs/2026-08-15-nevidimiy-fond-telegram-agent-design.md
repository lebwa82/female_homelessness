...
- `RiskAgent` возвращает строго типизированную оценку риска;
- `SupportAgent` читает всю сохранённую за 30 дней историю, доступный каталог,
  ...
  возвращает одно финальное типизированное действие.
...
Если PydanticAI проходит compatibility smoke test с OpenAI-compatible Responses
API Yandex AI Studio, он используется для типизированных output schemas. Если
конкретная версия PydanticAI добавляет несовместимые поля, транспорт заменяется
тонким адаптером над `openai.AsyncOpenAI`; типы skills/actions и application
interface не меняются. LangChain и LangGraph для этого одноходового графа не
нужны.
