...
- «Вернуться на шаг назад» восстанавливает предыдущее состояние и его поля, не удаляя переписку, события, заявки или фолоуапы;
...
- критический риск перекрывает обычный сценарий; для суицидального кризиса неизменно показан `8 (800) 100-49-94`;
...
История переходов хранится в `conversations.navigation` (JSONB) и переживает
перезапуск. «Назад» появляется, когда сохранено предыдущее состояние; повторный
клик по устаревшей кнопке не выполняет дополнительный откат. `/clear` начинает
новую цепочку навигации, сохраняя старые записи, но не возвращая их в контекст.
Переписка в одном состоянии не считается новым шагом. Для старых диалогов
цепочка переходов начнёт накапливаться после установки обновления.

Проверка навигации на локальном PostgreSQL (тестовые строки откатываются):
`TEST_POSTGRES_NAVIGATION=1 uv run pytest tests/test_navigation_postgres.py -q --tb=short`.
...
`just chatwoot-enable-polling` после деплоя актуального релиза.
`telegram-ingress` получает обновления через aiogram и существующий SOCKS-прокси,
передаёт их штатному внутреннему обработчику Chatwoot. История, кнопки,
ответы робота и дежурного остаются в Chatwoot. Новая БД не нужна.

- `/etc/women-help-telegram.env` (root, `0600`) содержит только настройки транспорта.
- `women-help-telegram-ingress.service` запускается при загрузке VM; Podman
  перезапускает упавший процесс. Старый `women-help-bot.service` должен быть выключен.
- В существующем Redis сохраняются offset, блокировка одного приёмника и время
  успешного опроса. Тексты сообщений приёмник не пишет в свои логи или ключи Redis.
- Подтверждение Telegram выполняется только после HTTP 200 от Chatwoot
  (событие принято в штатную очередь Sidekiq). Нормальный рестарт не повторяет
  подтверждённые обновления; сетевые ошибки повторяются с задержкой до 30 секунд.
- Доставка **at least once**, не exactly once: если Chatwoot принял запрос, но
  его HTTP-ответ потерялся, повтор может создать дубль. Его штатный Telegram
  endpoint не поддерживает ключ идемпотентности. Это отдельный технический долг.
- `just chatwoot-refresh-address` сохраняет выбранный transport=polling;
  изменение IP больше не влияет на получение сообщений из Telegram. Публичные
  адреса панели и Agent Bot всё ещё нужно обновлять этой командой.

Проверка приёмника без чтения сообщений: `sudo podman exec
women-help-chatwoot_telegram-ingress_1 /app/.venv/bin/python -m
app.chatwoot.telegram_ingress --check`. Код 0 означает успешный опрос за последние
120 секунд. Проверка не вызывает `getUpdates` и не конкурирует с приёмником.

Возврат к webhook: остановить и отключить `women-help-telegram-ingress.service`,
установить `TELEGRAM_UPDATE_TRANSPORT=webhook` в `/etc/women-help-chatwoot.env`,
затем выполнить `just chatwoot-refresh-address`. Ожидающие обновления не удаляются.

### Хранилище и первый запуск
...
сервисы, выполняет миграции и проверяет HTTPS. Обычная перезагрузка VM не
скачивает обновления. Деплой можно повторять для одного Git-коммита: он снова
проверит `latest` и выведет фактическую версию и ID запущенного образа.
PostgreSQL и Redis остаются на закреплённых версиях.

Перед обновлением сохраните дамп PostgreSQL, файлы конфигурации и данные Redis
и вложений. Для отката после миграций может понадобиться восстановление БД,
а не только возврат старого образа. Резервные копии на той же VM защищают от
неудачного обновления, но не от потери самой VM.

Прежний `v4.12.1` был выбран из-за регрессии создания Agent Bot. Переход на
`latest` сам по себе не завершает подключение бота: перед его включением
нужно проверить создание через API и задать выданный Chatwoot
`CHATWOOT_WEBHOOK_HMAC_SECRET`. В этом режиме приёмник принимает только
подписанные доставки; отдельный секрет в URL сохраняется.
...
inbox Chatwoot. При эскалации Agent Bot уведомляет команду приватным упоминанием
и отмечает `handoff_requested=true`, продолжая разговор до назначения сотрудницы.
`reply_owner` автоматически отражает именно назначение человека, а не уведомление
или назначение команды. Чтобы подключиться, дежурная назначает разговор себе;
чтобы вернуть боту — снимает назначение и при необходимости возвращает `pending`.
`/clear` и `/system_info` доступны и при человеке, но не отбирают у него разговор.
...
Сертификат на продукты, лекарства, проживание или товары для детей выдаётся
после отдельного подтверждения — не чаще одного на Chatwoot-контакт среди всех
четырёх категорий. `/clear` и новый разговор того же контакта не сбрасывают
этот факт: он хранится в PostgreSQL контура Chatwoot. Консультации психолога и
