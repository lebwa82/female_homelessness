...
Электронные сертификаты выдаются сразу после выбора карточки, без запроса
контакта. Выдача однократная: бот не проверяет использование и не перевыпускает
код. Запас хранится в PostgreSQL контура Chatwoot, а коды не возвращаются в
контекст модели.
