...
Все команды подключения получают свежий IPv4 через `yc` по постоянному ID VM
`epdqi9moqfn8fqbe3g2n` в каталоге `b1gepl5pmqr8f7hbu3bj`; IP не кешируется.
Нужен настроенный доступ Yandex Cloud CLI к этой VM. После смены её адреса:

```bash
just chatwoot-refresh-address
```
