...
Перед деплоем запустите `just check`, `just scenario-smoke`, `just llm-health`
и `just eval-dialogues`. Команда `just eval-dialogues-live` прогоняет
обезличенный набор против настроенной Qwen и расходует оплачиваемые токены Yandex
AI Studio; запускайте её сознательно после проверки ключа. Она печатает только
ID сценариев, итоговые классификации и нарушения инвариантов — не истории
диалогов.
...
только backend через `ConversationPolicy` после проверки `SupportPlan`.
